@echo off
REM Script para hacer push del fix de Prisma Client
REM Cambio: postbuild -> prebuild

echo ========================================
echo   Push Fix: Prisma Client en Vercel
echo ========================================
echo.
...
echo [INFO] Agregando archivo modificado...
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo - package.json: postbuild -> prebuild
    echo.
    echo Esto asegura que Prisma Client se genere ANTES del build
    echo en lugar de DESPUÉS, solucionando el error en Vercel.
    echo.
    echo El workflow de Vercel se ejecutará automáticamente.
    echo.
) else (
    echo.
