# Script PowerShell para subir el proyecto a GitHub (con datos preconfigurados)
# Uso: .\subir_a_github_automatico.ps1
...
# Datos preconfigurados (configura tu token aquí)
$GITHUB_TOKEN = "TU_TOKEN_DE_GITHUB_AQUI"
$GITHUB_USER = "joamercu"
$GITHUB_EMAIL = "joamercuia@gmail.com"

# Solicitar solo el nombre del repositorio
Write-Host "[INFO] Usuario de GitHub: $GITHUB_USER" -ForegroundColor Cyan
Write-Host "[INFO] Email: $GITHUB_EMAIL" -ForegroundColor Cyan
Write-Host ""
$GITHUB_REPO = Read-Host "Ingresa el nombre de tu repositorio (o presiona Enter para usar 'weldtech-app')"
if ([string]::IsNullOrWhiteSpace($GITHUB_REPO)) {
    $GITHUB_REPO = "weldtech-app"
}
...
Write-Host "[INFO] Configurando usuario de Git..." -ForegroundColor Yellow
git config user.name $GITHUB_USER
git config user.email $GITHUB_EMAIL
Write-Host "[OK] Usuario de Git configurado" -ForegroundColor Green
Write-Host ""
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "1. Crea el repositorio en GitHub primero: https://github.com/new" -ForegroundColor Yellow
