...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Log del intento de login desde el cliente
      console.log('🔍 [CLIENT] Intento de login iniciado:', { email, timestamp: new Date().toISOString() });
      ...
      if (result?.error) {
        console.error('❌ [CLIENT] Error en login:', { email, error: result.error, timestamp: new Date().toISOString() });
        setError('Email o contraseña incorrectos');
        setIsLoading(false);
        return;
      }

      console.log('✅ [CLIENT] Login exitoso, redirigiendo:', { email, timestamp: new Date().toISOString() });
      router.replace('/');
    } catch (err) {
      console.error('❌ [CLIENT] Error inesperado en login:', { email, error: err, timestamp: new Date().toISOString() });
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      console.log('🔍 [CLIENT] Intento de login con Google iniciado:', { timestamp: new Date().toISOString() });
      await signIn('google', { callbackUrl: '/' });
    } catch (err) {
      console.error('❌ [CLIENT] Error en login con Google:', { error: err, timestamp: new Date().toISOString() });
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">
            Accede sin marcas de agua en tus descargas
          </p>
        </div>
